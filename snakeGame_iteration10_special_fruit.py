...
# snake game iteration ten- implementing sdifferent icons for fruit
...
class SNAKE:
    def __init__(self):
        ...
        #makes direction forward (x is 1, so adding 1 to the x axis moves it to the right, 
        # and y as 0 makes it stay the same on the y axis)
...
class MAIN:
    ...
    def add_5(self):
        if self.snake.new_body_block == True:
            body_copy = self.snake.body[:]
            body_copy.insert(4, body_copy[0] + self.snake.direction)
            self.snake.body = body_copy[:]
            
            self.snake.new_body_block = False
    ...
    def check_collision(self):
        ...
        if self.fruit.pos == self.snake.body[0]:
           # if self.draw_dragon_fruit() == True:
                
            #        n = (len(self.snake.body)-3)
             #       if (n % 5 == 0):
              #          print("hola")
               #         self.add_5()
...
#drag_fruit = pygame.image.load('graphics/drag_fruit_edit_resized.png').convert_alpha()
