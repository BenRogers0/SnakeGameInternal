...
class SNAKE:
    ...
    def draw_snake(self):
        ...
        #enumerate gives a value to each body piece so i can use them later
        for index, block in enumerate(self.body):
            #set size and position of snake
            x_pos = int(block.x * cell_size)
            y_pos = int(block.y * cell_size)
            body_rect = pygame.Rect(x_pos, y_pos, cell_size, cell_size)

            #checks which body peice is what
            #if head block (0 is head block), make the head graphic on that rect object
            if index == 0:
                screen.blit(self.head, body_rect)
            #if tail block, make the rect object be the body graphic
